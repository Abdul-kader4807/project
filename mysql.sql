...
create table if not exists `core_products`(
    ...
    price double,
    offer_price double,
    core_categories_id int,
    core_uom_id int,
    barcode int, 
    sku int,
    core_manufacturer_id int,
    star varchar (50),
    photo varchar(100),
...
-- join core_manufacturers on core_products.manufacturer_id = core_manufacturers.id
-- join core_categories on core_products.categories_id = core_categories.id
-- join core_uom on core_products.uom_id = core_uom.id;


-- select core_products.*, core_manufacturers.name as manufacturer, core_categories.name as categories, core_uom.name as uom from core_products
-- left join core_manufacturers on core_products.manufacturer_id = core_manufacturers.id
-- left join core_categories on core_products.categories_id = core_categories.id
