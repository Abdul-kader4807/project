...
class Uom extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}uom(name,created_at,updated_at)values('$this->name','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}uom set name='$this->name',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	public static function delete($id){
		global $db,$tx;
		$db->query("delete from {$tx}uom where id={$id}");
	}
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,created_at,updated_at from {$tx}uom $criteria limit $top,$perpage");
		$data=[];
		while($uom=$result->fetch_object()){
			$data[]=$uom;
		}
	...
	public static function count($criteria=""){
		global $db,$tx;
		$result =$db->query("select count(*) from {$tx}uom $criteria");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$count_result =$db->query("select count(*) total from {$tx}uom $criteria ");
